...
class SCPN(nn.Module):
    ...
    def encode_transformations(self, trans, lengths, return_last=True):
        ...
        ##print 'd_trans: ', self.d_trans
        ##print 'bsz: ', bsz
        ##print 'trans_hid_init: ', self.trans_hid_init
        t_hid_init = self.trans_hid_init.expand(1, bsz, self.d_trans).contiguous()
        #print 't_hint_init: ', t_hid_init 
        ##print 'bsz: ', bsz
        ...
        while True:
            ...
            for b in beams:
                ...
                for z in range(beam_size):
                    word_idx = top_indices[z].data
                    beam_candidates.append((curr_prob + final_preds[word_idx].data, 
                        hn, cn, seq + [word_idx]))
        ...
        while True:
            ...
            for b_idx in beam_dict:
                ...
                if num_examples == len(beam_dict):
                    ...
                    for z in range(beam_size):
                        word_idx = top_indices[z].data
                        beam_candidates.append((preds[word_idx].data, 
                    ...
                    for o_idx, ob in enumerate(origin_beams):
                        ...
                        for z in range(beam_size):
                            ...
                            beam_candidates.append((curr_prob + float(preds[word_idx].cpu().data), 
...
if __name__ == '__main__':
    ...
    print "ignore zero targets in loss function"
    ...
    for ep in range(n_epochs):
        ...
        print "len: ", len(train_minibatches)
        for b_idx, (start, end) in enumerate(train_minibatches):
            ##if b_idx > 10000:
              ##  break
            
            if b_idx % 100 == 0:
                print "read data from hdf5", b_idx
            # read data from hdf5
            ...
            ##print "compute max output length"
            ...
            ##print "sentences are too short"
            # sentences are too short
            if max_in_len < args.min_sent_length:
                continue
            
            ##print "downsample if input sentence are too short"
            ...
            ##print "if training, compute loss and backprop"
            # if training, compute loss and backprop
            if not args.eval_mode:
                ##print "compute masked loss"
                # compute masked loss
            ...
            if args.eval_mode or (b_idx % args.save_freq == 0):
                ...
                ##print "curr_bsz: ", curr_bsz
                for i in range(min(3, curr_bsz)):
                    ##print "hack around beam search bug"    
                    ...
                    try:
                        ...
                        for beam_idx, beam in enumerate(beams):
                            ...
                            print 'gpu beam %d, score:%f: %s' % \
                                (beam_idx, beam[0], ' '.join([rev_vocab[int(w)] for w in beam[-1]]))
                            
                    except:
                        print 'beam search error'
                
                print "print statistics about the batch"
        ...
        print "adjust LR for next epoch"
