...
import torch, time, sys, argparse, codecs, h5py, cPickle, csv
import numpy as np
...
def encode_data(out_file):
    ...
    for d_idx, ex in enumerate(inrdr):
        ...
        beam_dict = parse_net.batch_beam_search(torch_parse.unsqueeze(0), tp_templates,
            torch_parse_len[:], tp_template_lens, parse_gen_voc['EOP'], beam_size=4, max_steps=150)
        #print 'beam_dict 0:', len(beam_dict), beam_dict
        seq_lens = []
        seqs = []
